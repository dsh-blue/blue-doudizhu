...
export function apply(ctx) {
      ...
      const COMBO_TYPES = ['single', 'pair', 'triple', 'triple1', 'triple2', 'straight', 'doubleSeq', 'plane', 'four2', 'bomb', 'rocket']
      ...
      function statusText(st) {
        if (st.thinking) return { text: centerLine(`🤔 ${st.thinkName} 思考中… 剩 ${remainSec(st)}s（最多等 ${THINK_TIMEOUT_MS / 1000}s）`), tone: 'default' }
        if (st.stage === 'ended') {
          const won = st.winner === st.players[0].role
          return { text: centerLine(`★★ ${won ? '你赢了！' : '你输了！'}（${st.winner === 'landlord' ? '地主获胜' : '农民获胜'}）★★`), tone: won ? 'success' : 'danger' }
        }
        if (st.currentIdx === 0) return { text: centerLine('→ 轮到你了！  出牌：/poker <编码>   不出：/poker p   /poker end'), tone: 'accent' }
        return { text: centerLine(`→ ${st.players[st.currentIdx].name} 出牌中…`), tone: 'default' }
      }
      function renderBoard(st) {
        ...
        lines.push('')
        lines.push(centerLine(`你的手牌 [${me.role === 'landlord' ? '地主' : '农民'}] 剩${me.hand.length}张：`))
        const hand = me.hand.slice().sort((a, b) => a.v - b.v)
        for (let i = 0; i < hand.length; i += HAND_CHUNK) {
          const chunk = hand.slice(i, i + HAND_CHUNK)
          for (const row of cardRow(chunk)) lines.push(centerLine(row))
        }
        lines.push('─'.repeat(BOARD_W))
        ...
        sections.push({ title: '', body: { kind: 'code', code: lines.join('\n'), language: '' } })
      ...
      function notify(text, tone) {
        if (notifications) notifications.publish({ id: 'ddz-note-' + Math.random().toString(36).slice(2), view: { kind: 'text', content: text, tone: tone || 'default' } })
      }

      async function drive() {
        const token = ++driverToken
        if (!state) return
        if (state.stage === 'ended') { if (token === driverToken) { settle(); notify(state.winner === 'landlord' ? '地主获胜！' : '农民获胜！', 'success') } return }
        ...
        if (state && state.stage === 'ended' && token === driverToken) { settle(); notify(state.winner === 'landlord' ? '地主获胜！' : '农民获胜！', 'success') }
      }
      ...
      async function execute(args) {
        if (!args || args.length === 0) { notify('/poker new 开始一局；/poker help 查看说明'); return { ok: true } }
        ...
        if (first === 'help' || first === 'rules') {
          notify('/poker new 开局；出牌 /poker <编码>；不出 /poker p。记分 /poker score；结束比赛看排行 /poker end；记牌器 /poker memo；Bot 模式 /poker bot；编码 0=10 jqka2 sx=王炸', 'muted')
          return { ok: true }
        }
        if (first === 'status') { notify(state ? turnDesc() : '当前无牌局'); return { ok: true } }
        if (first === 'score') { notify(`局分：${match.scores.map((s, i) => NAMES[i] + ' ' + s).join('  |  ')}（共 ${match.games} 局）`, 'muted'); return { ok: true } }
        if (first === 'quit' || first === 'exit') { cancelDriver(); stopTheThink(); state = null; notify('牌局已结束（不计分）', 'muted'); return { ok: true } }
        if (first === 'pause') { paused = true; stopTheThink(); if (dockSlot) { dockSlot.dispose(); dockSlot = null } notify('已暂停：牌面已收起；/poker resume 恢复并展开', 'muted'); return { ok: true } }
        if (first === 'resume') { paused = false; if (!dockSlot) { const r = registerDock(); if (r.ok) dockSlot = r.value } notify('已恢复，牌面展开', 'success'); drive(); return { ok: true } }
        ...
        if (first === 'heuristic' || first === 'bot') { botMode = first === 'heuristic' ? 'heuristic' : 'hybrid'; notify(`Bot 决策模式：${botMode}${botMode === 'hybrid' ? '（模型:' + (modelLabel() || '未配置') + '）' : ''}`, 'muted'); return { ok: true } }
